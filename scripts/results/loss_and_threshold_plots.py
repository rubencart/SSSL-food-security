...
def pretrain_run_plot(
    pt_run_grps, metric, title, rf_score_bal, rf_score, maj_bas_score
):
    plt.close()
...
def loss_type_plot(
    loss_grps, metric, frz, title, rf_score_bal, rf_score, maj_bas_score, clf_head
):
    plt.close()
    fig = plt.figure(figsize=(14, 10))
    ax = fig.gca()
    ...
    loss_means = loss_grps.mean()
    loss_stds = loss_grps.std()

    for is_sssl in (True, False):
        #                     freeze=True
        mns = loss_means.loc[(clf_head, is_sssl), metric]
        stds = loss_stds.loc[(clf_head, is_sssl), metric]
...
def limit_plot(df, metrics, glob_metric, fn, title, maj_bas_score):
    ...
    ax.set_prop_cycle("color", colors)
    sum_df = df.groupby(["pretrain_tlimit", "pretrain_slimit"], dropna=False)[
        metrics
    ].max()
    ...
    plt.ylabel(METRIC_NAME_DICT.get(glob_metric, glob_metric), fontsize=16)
    plt.xlabel("$(D_t, D_g)$ in months and degrees of lat/lon", fontsize=16)
    plt.title(title.replace("test_", "").replace("_", " "), fontsize=16)
    plt.legend(
        [
            "Majority baseline",
            "Majority voting",
...
def run():
    ...
    m13 = pd.isna(df["finetune/n_steps_in_future"]) | (
        df["finetune/n_steps_in_future"] == 0
    )
    ...
    all_df["pretrain_loss_type_sssl"] = all_df[
        "finetune/all_backbone_ckpts_in_dir"
    ].str.contains("sssl")
    all_df["pretrain_cfg_name"] = all_df[
        "finetune/all_backbone_ckpts_in_dir"
    ].str.extract(r"output\/[0-9_]+s[0-9]+_([0-9a-zA-Z_]+)\/", expand=False)
    ...
    all_df["pretrain_slimit"] = all_df[
        "finetune/all_backbone_ckpts_in_dir"
    ].str.extract(r"_s([0-9]+)_(?:RGB|ALL)", expand=False)
    ...
    for frz in (True,):  # (True, False):
        for bands in ("ALL", "RGB"):
            ...
            for metrics, rf_metric, maj_baseline_metric in (
                ...
                rf_score = rf_df[rf_df["random_forest/class_weight"].isnull()][
                    rf_metric
                ].iloc[0]
                ...
                for ltype in (
                    ...
                    try:
                        ...
                        print(
                            "Not enough completed runs for pairs plot for frz %s, loss type %s, bands %s, metric %s"
                            % (frz, ltype, bands, metrics[0])
                        )
                ...
                for metric in metrics:
                    for head in ("linear", "mlp"):
                        try:
                            ...
                            print(
                                "Not enough completed runs for loss_type plot for frz %s, metric %s, bands %s, head %s"
                                % (frz, metric, bands, head)
                            )
                    ...
                    for tlimit in (1, 4, 12, 36, 84):
                        try:
                            ...
                            print(
                                "Not enough completed runs for pt run plot for frz %s, tlimit %s, metric %s, bands %s"
                                % (frz, tlimit, metric, bands)
                            )
